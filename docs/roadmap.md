...
Milestones M1 to M4 are complete, merged, and live on the sandbox. This page
records what is done, plus a deferred backlog of candidate next steps. None of
...
| **M4** | Deterministic eval harness (`contact-center eval`, 14/14 as of 2026-09-11) + PII-safe, `session_id`-keyed structured logging |

## Hardening since M4

Work merged after the milestone set closed. Both items deepen what M2 and M3
already delivered, without changing scope.

| PR | Change |
|----|--------|
| **#5** Connect conversation lifecycle | The `--connect` harness transport is now a `ConnectConversation` context manager that owns the contact, websocket and transcript cursor. A turn is `send()` then `wait()`, returning a `TurnResult` (`messages`, `transferred`, `ended`); polling and printing are separated, and the socket closes deterministically. |
| **#6** Strict response contract | `SupervisorResponse` replaces the permissive parser: exact field set, real `bool` for `escalate`, non-empty `answer`, `reason` consistent with `escalate`. The fallback flipped from passing raw model text through (`escalate=false`) to a fixed answer with `escalate=true` and `reason="Keine gesicherte Antwort möglich"`. The bridge Lambda re-validates independently. |

The contract change is the one with regulatory weight: before it, an
off-contract model response could be rendered to a customer verbatim.

It also had a regression, found by re-running the eval on 2026-09-11 and since
fixed. A Bedrock Guardrail replaces the blocked turn with its own plain-text
message, which cannot satisfy the JSON contract, so every compliance refusal was
rejected and turned into a handoff and the documented refusal never reached the
customer. The supervisor now checks Strands' `stop_reason` for
`guardrail_intervened` / `content_filtered` and passes the guardrail's message
through as a non-escalating answer. Contract rejections are logged now as well
(`contract_rejected`, cause and field names, no answer text). They were swallowed
silently before, which is why the regression survived a merge.

## Known regulatory and security limitations

This is a PoC on a sandbox account. Before any regulated go-live, the following
gaps are open and must be closed. Several of them are the reason the backlog
items below exist.
...
- **Compliance controls depend on a probabilistic guardrail.** PII anonymization
  and the investment-advice refusal are enforced by a Bedrock Guardrail, which is
  a model-based control rather than a deterministic one. A missed refusal or a
  leaked PII token is possible. Acceptable for a PoC; a regulated deployment needs
...
- **No audit-grade trail or retention policy.** Correlation logs are PII-safe and
  `session_id`-keyed, but were built for operational tracing rather than BaFin/DORA
...
Data residency is in place (region `eu-central-1`, EU-hosted model and Knowledge
Base), so it is not among the open gaps.
...
- **Gateway JWT authorizer identity**: replace the client-asserted `customer_id`
  (today a Connect contact attribute) with a Cognito-verified token claim. This
  closes the last identity gap and is what a regulated deployment actually gates
  on. Backend-only, no new UI.
...
- **Voice channel**: claim a phone number, add a voice contact flow, and use Lex
  speech (Polly) over the same bridge Lambda. Biggest demo impact and biggest
  scope; reuses the M3 plumbing.
- **Hosted chat widget**: a customer-facing web chat UI (via a thin backend plus
...
- **Production-readiness** (JWT identity, staffed workspace): what blocks a real
  ...
  authorizer identity is the highest-leverage next step: it is the one remaining
  gap between "demo" and "a bank could run this."
- **Demo expansion** (voice, widget): increases reach and impressiveness, but
  adds scope rather than trust.
