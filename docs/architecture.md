...
All infrastructure runs in AWS `eu-central-1`, in a single sandbox account, and
no data leaves the region. The sections below cover the runtime components, the
path a chat turn takes, knowledge base ingestion, escalation, and observability.

## System context

An operator drives the agent directly through the CLI harness. A customer
reaches it through Amazon Connect chat. Both paths end at the supervisor agent
on Bedrock AgentCore Runtime, which is grounded by a Knowledge Base and can look
up balances through an AgentCore Gateway.
...
|-----------|-------|----------------|
| **CLI harness** | `src/contact_center/` | `chat` (direct or `--connect`) and `eval` subcommands; talks to the runtime via boto3 and to Connect through `ConnectConversation` (one `send()`/`wait()` per turn, returning a `TurnResult`) |
...
| **Response contract** | `contract.py` | `SupervisorResponse.from_supervisor_output` applies strict validation of `{answer, escalate, reason}`; malformed output fails toward a human instead of reaching the customer. `guardrail_refusal` carries a guardrail-blocked turn through as a non-escalating answer |
...
| **Connect + Lex + bridge** | `infra/lib/connect-stack.ts`, `infra/lambda/bridge/` | Chat front door; Lex as a dumb pipe; bridge forwards turns to the runtime and re-validates the contract independently (`_is_valid_agent_response`) |
...
sequenceDiagram
  ...
  alt escalate = false
    ...
    BR-->>LX: Close (fixed fallback text)
    CN->>Q: transfer to human queue
  end
```
...
The third branch is the fail-closed path. A runtime response that violates the
contract (wrong field set, non-boolean `escalate`, empty `answer`, unknown
`reason`) is never rendered to the customer as raw text. The agent turns it into
an escalation with `reason="Keine gesicherte Antwort möglich"`. If the response
never reaches that check, through a runtime error or an invalid payload, the
bridge applies its own fallback with `reason="Systemfehler"`. Validation happens
twice, independently, on both sides of `invoke_agent_runtime`.
...
- **Guardrail at the model boundary.** PII is anonymized; investment-advice
  requests are denied (regulatory: no unlicensed advice). A blocked turn arrives
  as the guardrail's own plain-text message, which cannot satisfy the JSON
  contract, so the supervisor recognizes it by stop reason and delivers the
  refusal to the customer instead of escalating.
